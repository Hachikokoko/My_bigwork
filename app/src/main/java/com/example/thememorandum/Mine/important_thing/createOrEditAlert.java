...
public class createOrEditAlert extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.important_editalert);
        ...
        if (mData != null) {
            ...
            setActionBarTitle(actionBar, "添加备忘");
        }
        else {
    ...
    private void createAlarm(int id) {
        Intent alarm = new Intent(this, AlarmService02.class);
        alarm.putExtra(ReminderParams.ID, id);
        alarm.setAction(AlarmService02.CREATE);
        startService(alarm);
    }
    ...
    private void deleteAlert(ReminderItem item) {
        if (item != null) {
            Intent delete = new Intent(createOrEditAlert.this, AlarmService02.class);
            delete.putExtra(ReminderParams.ID, item.getId());
            delete.setAction(AlarmService02.DELETE);
    ...
    private void saveAlert(final ReminderItem item) {
        if (item.getId() > 0) {
            Intent cancelPrevious = new Intent(createOrEditAlert.this,
                    AlarmService02.class);
            cancelPrevious.putExtra(ReminderParams.ID, item.getId());
            cancelPrevious.setAction(AlarmService02.CANCEL);
